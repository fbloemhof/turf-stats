...
function turf_analyse_register_metaboxes() {
	...
	add_meta_box( 'turf_search_terms', __( 'Search terms', 'turf-stats' ), function () use ( $days ) {
		turf_search_render_top_terms( $days );
	}, $hook, 'normal' );

	add_meta_box( 'turf_search_zero_results', __( 'Zero-result searches', 'turf-stats' ), function () use ( $days ) {
		turf_search_render_zero_results( $days );
	}, $hook, 'normal' );

	add_meta_box( 'turf_session_routes', __( 'Visitor routes', 'turf-stats' ), function () use ( $days ) {
	...
	add_meta_box( 'turf_authors', __( 'By author', 'turf-stats' ), function () use ( $days ) {
		turf_render_author_breakdown( $days );
	}, $hook, 'normal' );

	add_meta_box( 'turf_forms', __( 'Forms', 'turf-stats' ), function () use ( $days ) {
